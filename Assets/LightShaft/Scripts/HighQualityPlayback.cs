...
public class HighQualityPlayback : MonoBehaviour {
    ...
    void FixedUpdate(){
      ...
      //  PlayerTime = VideoPlayer.GetSeekBarValue();
        }
        ...
        if (videoAreReadyToPlay) {
			...
			if (!useNewUnityPlayer)
				...
				unityVideoPlayer.Prepare ();
                    unityVideoPlayer.time = PlayerTime;
                   VR = false;
                    unityVideoPlayer.Play();
                }
                else {
              
                VideoPlayer.m_strFileName = videoUrl;
                    YoutubeVideoURl = videoId;
                    videoId = videoUrl;
